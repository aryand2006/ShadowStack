...
/** Replaces deprecated Long constructors. */
public class LongConstructorRule implements RefactorRule {
    private static final String RULE_ID = "LONG_CTOR_TO_VALUEOF";
    private static final Set<String> TYPES = Set.of("Long");
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "new Long(x) → Long.valueOf"; }
    @Override public String description() { return "Replaces deprecated Long constructors."; }
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "Long.valueOf(" + arg0 + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("long_valueof", "valueOf replaces Long constructors", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Long constructors are deprecated")
                .astNode(node).build();
    }
