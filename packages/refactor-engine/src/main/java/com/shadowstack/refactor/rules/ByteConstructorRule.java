...
/** Replaces deprecated Byte constructors. */
public class ByteConstructorRule implements RefactorRule {
    private static final String RULE_ID = "BYTE_CTOR_TO_VALUEOF";
    private static final Set<String> TYPES = Set.of("Byte");
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "new Byte(x) → Byte.valueOf"; }
    @Override public String description() { return "Replaces deprecated Byte constructors."; }
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "Byte.valueOf(" + arg0 + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("byte_valueof", "valueOf replaces Byte constructors", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Byte constructors are deprecated")
                .astNode(node).build();
    }
