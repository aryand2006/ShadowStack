...
/** Replaces Collections.singletonList(x) with List.of(x). */
public class CollectionsSingletonListToListOfRule implements RefactorRule {
    private static final String RULE_ID = "COLLECTIONS_SINGLETONLIST_TO_LISTOF";
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "Collections.singletonList → List.of"; }
    @Override public String description() { return "Replaces Collections.singletonList(x) with List.of(x)."; }
    ...
    public boolean appliesTo(ASTNode node, SemanticContext context) {
        ...
        if (!Set.of("singletonList").contains(name)) return false;
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        invariants.add(SafetyInvariant.verified("list_of_one", "List.of(x) replaces singletonList", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Prefer List.of for singleton immutable lists")
                .astNode(node).build();
    }
