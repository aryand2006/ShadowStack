...
/** Replaces deprecated Double constructors. */
public class DoubleConstructorRule implements RefactorRule {
    private static final String RULE_ID = "DOUBLE_CTOR_TO_VALUEOF";
    private static final Set<String> TYPES = Set.of("Double");
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "new Double(x) → Double.valueOf"; }
    @Override public String description() { return "Replaces deprecated Double constructors."; }
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "Double.valueOf(" + arg0 + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("double_valueof", "valueOf replaces Double constructors", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Double constructors are deprecated")
                .astNode(node).build();
    }
