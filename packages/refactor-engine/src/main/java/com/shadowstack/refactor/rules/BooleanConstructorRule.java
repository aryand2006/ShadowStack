...
/** Replaces deprecated Boolean constructors. */
public class BooleanConstructorRule implements RefactorRule {
    private static final String RULE_ID = "BOOLEAN_CTOR_TO_VALUEOF";
    private static final Set<String> TYPES = Set.of("Boolean");
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "new Boolean(x) → Boolean.valueOf/parse"; }
    @Override public String description() { return "Replaces deprecated Boolean constructors."; }
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "Boolean.valueOf(" + arg0 + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("boolean_valueof", "valueOf replaces Boolean constructors", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Boolean constructors are deprecated")
                .astNode(node).build();
    }
