...
/** Replaces Collections.emptyMap() with Map.of() (Java 9). */
public class CollectionsEmptyMapToMapOfRule implements RefactorRule {
    private static final String RULE_ID = "COLLECTIONS_EMPTYMAP_TO_MAPOF";
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "Collections.emptyMap → Map.of"; }
    @Override public String description() { return "Replaces Collections.emptyMap() with Map.of() (Java 9)."; }
    ...
    public boolean appliesTo(ASTNode node, SemanticContext context) {
        ...
        if (!Set.of("emptyMap").contains(name)) return false;
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "java.util.Map.of()";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("map_of_empty", "Map.of() is the modern empty immutable map", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Prefer Map.of() for empty immutable maps")
                .astNode(node).build();
    }
