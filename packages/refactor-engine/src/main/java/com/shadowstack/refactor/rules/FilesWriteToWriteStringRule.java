...
/** Suggests Files.writeString for text writes (Java 11). */
public class FilesWriteToWriteStringRule implements RefactorRule {
    private static final String RULE_ID = "FILES_WRITE_TO_WRITESTRING";
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "Files.write(path, bytes) → writeString"; }
    @Override public String description() { return "Suggests Files.writeString for text writes (Java 11)."; }
    ...
    public boolean appliesTo(ASTNode node, SemanticContext context) {
        ...
        if (!Set.of("write").contains(name)) return false;
        Expression expr = inv.getExpression();
        if (expr == null) return false;
        String recv = expr.toString();
        if (!("Files".equals(recv) || "java.nio.file.Files".equals(recv))) return false;
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "java.nio.file.Files.writeString(" + joined + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("write_string", "writeString encodes with explicit charset APIs", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Prefer Files.writeString for character data")
                .astNode(node).build();
    }
