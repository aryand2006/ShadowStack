...
/** Replaces deprecated Short constructors. */
public class ShortConstructorRule implements RefactorRule {
    private static final String RULE_ID = "SHORT_CTOR_TO_VALUEOF";
    private static final Set<String> TYPES = Set.of("Short");
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "new Short(x) → Short.valueOf"; }
    @Override public String description() { return "Replaces deprecated Short constructors."; }
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "Short.valueOf(" + arg0 + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("short_valueof", "valueOf replaces Short constructors", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Short constructors are deprecated")
                .astNode(node).build();
    }
