...
/** Replaces deprecated Character constructors. */
public class CharacterConstructorRule implements RefactorRule {
    private static final String RULE_ID = "CHARACTER_CTOR_TO_VALUEOF";
    private static final Set<String> TYPES = Set.of("Character");
    @Override public String ruleId() { return RULE_ID; }
    @Override public String ruleName() { return "new Character(c) → Character.valueOf"; }
    @Override public String description() { return "Replaces deprecated Character constructors."; }
    ...
    public RefactorCandidate analyze(ASTNode node, SemanticContext context) {
        ...
        String proposed = "Character.valueOf(" + arg0 + ")";
        int startLine = context.getLineNumber(node.getStartPosition());
        List<SafetyInvariant> invariants = new ArrayList<>();
        invariants.add(SafetyInvariant.verified("char_valueof", "valueOf replaces Character constructors", proposed));
        return RefactorCandidate.builder(RULE_ID, context.getSourceFilePath())
                ...
                .rationale("Character constructors are deprecated")
                .astNode(node).build();
    }
